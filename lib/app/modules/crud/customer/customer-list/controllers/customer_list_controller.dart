...
class CustomerListController extends BaseGetXController implements ICustomerListController {
  ...
  onCustomerListFetchDone(CustomerAccountListResponse customerListResponse) {
    ...
    if ((customerListResponse.customerList?.length ?? 0) > 0) {
      isLoading.value = false;
      customerList.value = [customerList, customerListResponse.customerList].expand((x) => x).toList();
    }
  }
