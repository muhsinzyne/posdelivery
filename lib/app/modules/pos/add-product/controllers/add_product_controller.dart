...
class AddProductController extends BaseGetXController implements IAddProductController {
  ...
  _calculations() {
    ...
    pInfo.value.itemTaxMethod = product.row.taxMethod;
  ...
  void reCalculate() {
    ...
    if (product.taxRate != null) {
      ...
      if (cUnit.operator != null && cUnit.operationValue != null) {
        pInfo.value.unitMulti = (pInfo.value.unitMulti * int.tryParse(cUnit.operationValue));
      }
    ...
    pInfo.value.totalTax = pInfo.value.taxVal * (pInfo.value.itemQty * pInfo.value.unitMulti);
